...
const menuLists = [
    ...
    { icon: payouts, name: "Payouts" },
    { icon: discounts, name: "Discounts" },
    { icon: audience, name: "Audience" },
    { icon: appearence, name: "Appearence" },
...
const Sidebar = () => {
    ...
    return (
        <>
            <aside className={`fixed lg:sticky left-0 top-0 z-10 h-[100vh] w-full overflow-hidden transition-all duration-300 lg:max-w-[224px] ${isOpen ? "pointer-events-all opacity-100" : "pointer-events-none opacity-0 lg:opacity-100 lg:pointer-events-auto"}`}>
                <div className="absolute inset-0 transition-opacity bg-gray-500 bg-opacity-75"></div>
                <button className={`${isOpen ? "fixed" : "hidden"} top-5 right-4 lg:hidden`} onClick={handleToggleModal}>
                    <IoIosCloseCircle className="text-white text-3xl" />
                </button>
                <div className={`relative mr-auto transition-transform duration-300 ease-in-out flex flex-col max-w-[224px] px-2 py-4 h-[100vh] gap-4 bg-[#1E2640] justify-between ${isOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`} ref={modalRef}>
                    <div className="w-full flex flex-col gap-6 items-center">
                        <div className="flex gap-3 text-[#FFFFFF] items-center w-fit mx-auto">
                            <div className="bg-white w-[39px] h-[39px] m-[0.5px] rounded-[4px] relative">
                                <Image src={nishyan} alt="nishyanLogo" fill className="object-cover object-center" quality={95} />
                            </div>
                            <div className="flex flex-col gap-1 w-[108px] justify-center">
                                <h3 className="font-medium text-[15px] leading-[22px]">Nishyan</h3>
                                <Link href={"#"} className="underline leading-[16px] text-[13px] font-extralight w-fit text-[#D2D4D9]">
                                    Visit Store
                                </Link>
                            </div>
                        ...
                        <div className="w-full flex-col gap-4">
                            {menuLists.map((item) => (
                                <Link key={item.name} href={"#"} className={`${item.name === "Payouts" ? "bg-[#FFFFFF]/10" : ""} px-4 py-2 flex gap-3 text-[#D2D4D9] items-center rounded-[4px] hover:bg-[#FFFFFF]/10`}>
                                    <div className="relative w-[18px] h-[18px]">
                                        <Image src={item.icon} alt={item.name} fill className="object-contain object-center" />
                                    </div>
